...
class ImageSaverNode:
    def __init__(self):
        ...
        self.image_topics = [
            topic.strip() for topic in image_topics_str.split(',') if topic.strip()]
        self.camera_info_topics = [
            topic.strip() for topic in camera_info_topics_str.split(',') if topic.strip()]
        ...
        for i in range(len(self.image_topics)):
            ...
            rospy.Subscriber(
                self.image_topics[i], Image, self.image_callback, i)
        ...
        for i in range(len(self.camera_info_topics)):
            ...
            rospy.Subscriber(
                self.camera_info_topics[i], CameraInfo, self.camera_info_callback, i)

        # Create a service to trigger the saving to a ROSbag
        self.close_bag_service = rospy.Service(
            'close_bag', Trigger, self.close_bag)
        self.save_service = rospy.Service(
            'save_snapshots_to_bag', Trigger, self.save_snapshots_to_bag)
        # create directory
        ...
        self.bag_filename = os.path.join(
            self.save_directory, f'images_{timestamp}.bag')
        ...
        try:
            ...
            for i in range(len(self.camera_info_topics)):
                if self.last_camera_infos[i] is not None:
                    ...
                    self.bag.write(
                        self.camera_info_topics[i], self.last_camera_infos[i])
        finally:
