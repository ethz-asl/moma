...
class SphericalSamplerState(StateRos):
    ...
    def __init__(self, ns, outcomes=['Completed', 'Failure']):
        ...
        self.phi_range = self.get_scoped_param(
            "phi_range", [0, np.pi / 2])  # Use upper hemisphere by default
        self.heading = self.get_scoped_param("heading", [1, 0, 0])
        self.pose_broadcaster = tf2_ros.StaticTransformBroadcaster()

    def run(self):
        T_obj_cam = spherical_to_pose(r=sample(self.radius_range),
                                      theta=sample(self.theta_range),
                                      phi=sample(self.theta_range),
                                      heading=self.heading)
        sample_pose = se3_to_transform(T_obj_cam, rospy.Time.now(),
                                       self.base_frame, self.sample_name)
...
class FOVSamplerState(StateRos):
    ...
    def __init__(self, ns, outcomes=['Completed', 'Failure']):
        ...
        self.camera_frame = self.get_scoped_param("camera_frame",
                                                  "camera_optical_frame")
        self.controlled_frame = self.get_scoped_param("controlled_frame",
                                                      "controlled_frame")
        ...
        self.camera_info_topic = self.get_scoped_param("camera_info_topic",
                                                       "/camera_info")
        self.heading = self.get_scoped_param("heading", [1, 0, 0])
    ...
    def _init_sample_poses(self):
        ...
        T_cam_ctrl = self.get_transform(self.camera_frame,
        ...
        for theta, phi in zip(thetas, phis):
            ...
            self.sample_poses.append(
                se3_to_transform(T_map_ctrl, rospy.Time.now(), self.map_frame,
                                 self.sample_name))
    ...
    def run(self):
        if not self.camera_info:
            rospy.logerr("No camera_info message received on {}".format(
        ...
        if self.attempts < self.num_samples:
            self.pose_broadcaster.sendTransform(
                self.sample_poses[self.attempts])
            self.attempts += 1
            rospy.sleep(2.0)
            return 'Completed'
        else:
