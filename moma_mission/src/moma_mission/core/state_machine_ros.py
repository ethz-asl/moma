...
class StateMachineRos(smach.StateMachine):
    ...
    def add(self, label, state, transitions=None, remapping=None):
        if isinstance(state, (StateMachineRos, smach.StateMachine)):
            ...
            super(StateMachineRos, self).add(label, state(ns=label), transitions, remapping)
        else:
            super(StateMachineRos, self).add(label, state, transitions, remapping)
...
if __name__ == "__main__":
    ...
    with state_machine:
        state_machine.add('STATE_A', StateRos, transitions={'Completed': 'STATE_B', 'Failure': 'Failure'})
        state_machine.add('STATE_B', StateRos, transitions={'Completed': 'SEQUENCE', 'Failure': 'Failure'})
        sequence = StateMachineRos(outcomes=['Sequence_DONE', 'Failure'])
        with sequence:
            sequence.add('STATE_C', StateRos, transitions={'Completed': 'Sequence_DONE', 'Failure': 'Failure'})
        state_machine.add('SEQUENCE', sequence, transitions={'Sequence_DONE': 'Success', 'Failure': 'Failure'})

    # run
    rospy.init_node('test_mission')
    introspection_server = smach_ros.IntrospectionServer('mission_server', state_machine, '/mission')
