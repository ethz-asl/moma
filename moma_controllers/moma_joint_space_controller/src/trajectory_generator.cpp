...
void TrajectoryGenerator::compute(const Eigen::VectorXd& start, const Eigen::VectorXd& end,
                                  double t_start) {
  initial_time = t_start;

  // check
  if (start.size() != generators_.size() || end.size() != generators_.size()) {
  ...
  max_time = -1.0;
  for (unsigned int i = 0; i < generators_.size(); i++)
    if (generators_[i]->Duration() > max_time) max_time = generators_[i]->Duration();
