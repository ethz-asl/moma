...
void VelocityProfile_Trap::SetProfileDuration(double pos1, double pos2, double newduration) {
  // duration should be longer than originally planned duration
  // Fastest :
  SetProfile(pos1, pos2);
  // Must be Slower  :

  if ((duration == newduration) || (duration < newduration)) return;

  double factor = duration / newduration;

  if (factor > 1) return;  // do not exceed max
  a2 *= factor;
  a3 *= factor * factor;
  b2 *= factor;
  b3 *= factor * factor;
...
void VelocityProfile_Trap::SetProfileVelocity(double pos1, double pos2, double newvelocity) {
  // Max velocity
  SetProfile(pos1, pos2);
  // Must be Slower  :
...
double VelocityProfile_Trap::Duration() const { return duration; }
