  ...
  publishingTimer_ =
      node_handle.createTimer(ros::Duration(0.02), [this](const ros::TimerEvent& timerEvent) {
...

        {
          ...
          tf::quaternionEigenToMsg(Eigen::Quaterniond(T_base_tool_desired.linear()),
                                   desiredPose.pose.orientation);
          ...
          tf::pointEigenToMsg(T_base_tool_desired_filtered.translation(),
                              desiredPoseFiltered.pose.position);
          ...
          error.data.push_back(
              (T_base_tool_desired_filtered.translation() - T_base_tool_.translation()).norm());
...
bool CartesianImpedanceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  if (!init_params(node_handle)) return false;
...
void CartesianImpedanceController::starting(const ros::Time& /*time*/) {
  ...
  if (sim_) {
    ...
    initial_transform =
        Eigen::Affine3d(model_->getFramePlacement(ee_frame_id_).toHomogeneousMatrix());
  } else {
...
void CartesianImpedanceController::update(const ros::Time& /*time*/,
  ...
  if (targetWrench.tail(3).norm() > params_.torqueLimit_) {
    targetWrench.tail(3) =
        targetWrench.tail(3) / targetWrench.tail(3).norm() * params_.torqueLimit_;
  }
  ...
  for (size_t i{}; i < 7; i++) {
    ...
    if (delta_abs > tol &&
        (tau_task[i] * delta) < 0) {  // change tau only if acts against the limit
