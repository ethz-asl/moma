...
bool UrMpcController::init(hardware_interface::VelocityJointInterface* hw, ros::NodeHandle& nh) {
  ...
  mpc_controller_ = std::unique_ptr<moma_controllers::MpcController<armInputDim_>>(
      new moma_controllers::MpcController<armInputDim_>(nh));
...
bool UrMpcController::init_parameters(ros::NodeHandle& nh) {
  if (!nh.getParam("joint_names", joint_names_) || joint_names_.size() != armInputDim_) {
    ROS_ERROR(
  ...
  if (!nh.getParam("odom_topic", odom_topic_)) {
    ...
    odom_sub_ = nh.subscribe(odom_topic_, 1, &UrMpcController::odom_callback, this);
  }
  ...
  for (size_t i = 0; i < armInputDim_; i++) {
    if (!arm_pid_controllers_[i].init(
            ros::NodeHandle(nh, nh.getNamespace() + "/gains/" + joint_names_[i]), false)) {
  ...
  if (!nh.getParam("measurement_trust_factor", measurement_trust_factor_)) {
    ROS_INFO_STREAM("UrMpcController: measurement_trust_factor not found. Defaulting to "
...
void UrMpcController::starting(const ros::Time& time) {
  ...
  ROS_DEBUG_STREAM("[UrMpcController::starting] Starting with current joint position: "
                   << position_current_.transpose());
  mpc_controller_->start(
      position_current_.head<ocs2::mobile_manipulator::STATE_DIM(armInputDim_)>());
...
void UrMpcController::compute_command(const ros::Duration& period) {
  ...
  for (int i = 0; i < armInputDim_; i++) {
    arm_vel[i] = arm_pid_controllers_[i].computeCommand(
...
void UrMpcController::update(const ros::Time& time, const ros::Duration& period) {
  read_state();
  // ROS_INFO_STREAM_THROTTLE(0.5, "State is: " <<
  // position_current_.head<ocs2::mobile_manipulator::STATE_DIM(armInputDim_)>());
  mpc_controller_->update(
      time, position_current_.head<ocs2::mobile_manipulator::STATE_DIM(armInputDim_)>());
...
PLUGINLIB_EXPORT_CLASS(moma_controllers::UrMpcController, controller_interface::ControllerBase)
