...
class DummyActionServers:
    def __init__(self):
        ...
        self.action_server_drop_move = actionlib.SimpleActionServer(
            "drop_move_action",
            DropMoveAction,
            execute_cb=self.drop_cb,
            auto_start=False,
        )
        self.action_server_drop_move.start()
        rospy.loginfo("Drop move action server running.")
    ...
    def drop_cb(self, goal):
        ...
        rospy.sleep(1.0)
        rospy.loginfo("Success")
        self.action_server_drop_move.set_succeeded(result)
