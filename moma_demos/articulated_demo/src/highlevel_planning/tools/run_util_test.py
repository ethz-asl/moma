...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default="direct",				# To display gui put 'gui'
    )
...
def setup_pybullet_world(scene_object, basedir, savedir, objects, args, cfg, robot_mdl, graspOffset, elementToAdd):		# Added grasp offset
    # Create world
    world = WorldPybullet(
        style=args.method,
        sleep=args.sleep,
    ...
    scene = scene_object(world, basedir, graspOffset, elementToAdd, restored_objects=objects)					# graspOffset is given to scene_object
