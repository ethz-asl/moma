...
def construct_state_machine():
    ...
    with sm:
        ...
        # TODO fix this, it doesnt work with the return values of 0,1,2, make dependet on self.obj_id
        StateMachine.add(
        ...
        StateMachine.add(
            "MOVE_TO_OBJECT_0",
            ServiceState("move_to_object_id", MoveToObjID, request=0), #PW TODO change to single state with a varabile for response 
            transitions={
                "succeeded": "WAIT_FOR_MODEL",
            },
        )
        ...
        StateMachine.add(
            "WAIT_FOR_MODEL",
            ServiceState("wait_for_model", GetTowerPrediction, response_slots=["y_offset"]),
        ...
        # TODO correctly add here the obj_id logic
        StateMachine.add(
            "RETURN_OBJECT",
