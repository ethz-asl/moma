...
class object_tracker:
  ...
  def __init__(self):
    ...
    self.BB_sub = rospy.Subscriber("/CVdetectorAction/result",DetectionActionResult,self.callbackBB)
  ...
  def callbackBB(self,data):
    if data.result.success:
      self.initBB = (data.result.targetBB[0],data.result.targetBB[3],abs(data.result.targetBB[2] - data.result.targetBB[0]),abs(data.result.targetBB[3] - data.result.targetBB[1]))
      self.successTracking = False
      print(self.initBB)
