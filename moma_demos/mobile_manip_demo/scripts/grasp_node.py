...
from grasp_demo.msg import GraspAction, GraspGoal, ScanSceneAction, ScanSceneGoal
from sensor_msgs.msg import PointCloud2
from std_srvs.srv import Trigger

from mobile_manip_demo import environment as env
from mobile_manip_demo.skill_template import Skill
from mobile_manip_demo.msg import GraspAction, GraspResult
...
class GraspSkill(Skill):
    def __init__(self):
        ...
        # Action clients
        self.client_reconstruct = SimpleActionClient("scan_action", ScanSceneAction)
    ...
    def execute_callback(self, goal):
        ...
        resp = self.srv_reset()
        if not resp.success:
            self.report_failure(GraspResult(), "Calling the reset service failed")
            return
        rospy.loginfo("Scene resetted")

        # Reconstruct scene
        self.client_reconstruct.send_goal(ScanSceneGoal())
        rospy.loginfo("Reconstructing scene")
        res = self.wait_monitoring_preemption(self.client_reconstruct)
        if not res:
            self.report_preemption(
                GraspResult(), "Preempted during scene reconstruction"
            )
            return
        state = self.client_reconstruct.get_state()
        if state != GoalStatus.SUCCEEDED:
            self.report_failure(GraspResult(), "Scene reconstruction failed")
            return
        ...
        if num_grasps == 0:
            self.report_failure(GraspResult(), "No grasps detected")
            return
        ...
        if int(goal.goal_id) > -1:
            ...
            if msg is None:
                self.report_failure(GraspResult(), "Transformation Error")
                return
        ...
        grasp_goal = GraspGoal(target_grasp_pose=target_pose_msg)
        self.client_grasp_execution.send_goal(grasp_goal)
        res = self.wait_monitoring_preemption(self.client_grasp_execution)
        if not res:
            self.report_preemption(GraspResult(), "Preempted during grasp execution")
            return
        state = self.client_reconstruct.get_state()
        if state != GoalStatus.SUCCEEDED:
            self.report_failure(GraspResult(), "Grasp execution failed")
            return
        ...
        if response.success:
            self.report_success(GraspResult(), "Finished successfully")
        else:
            self.report_failure(GraspResult(), "Grasp execution failed")
