...
class DummyActionServers:
    def __init__(self):
        ...
        self.action_server_scan = actionlib.SimpleActionServer(action_name, ScanSceneAction, execute_cb=self.scan_cb, auto_start=False)
        self.action_server_scan.start()
        rospy.loginfo("Scan action server running.")

        action_name = 'grasp_action'
        self.action_server_grasp = actionlib.SimpleActionServer(action_name, GraspAction, execute_cb=self.grasp_cb, auto_start=False)
        self.action_server_grasp.start()
        rospy.loginfo("Grasp action server running.")

        action_name = 'drop_action'
        self.action_server_drop = actionlib.SimpleActionServer(action_name, DropAction, execute_cb=self.drop_cb, auto_start=False)
