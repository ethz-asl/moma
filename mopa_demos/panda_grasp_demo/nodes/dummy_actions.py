
import actionlib
from panda_grasp_demo.msg import ScanSceneAction, ScanSceneResult
from geometry_msgs.msg import PoseStamped
import rospy


class DummyActionServers:
    def __init__(self):
        action_name = 'pointcloud_scan_action'
        self.action_server_scan = actionlib.SimpleActionServer(action_name, ScanSceneAction, execute_cb=self.scan_cb, auto_start=False)
        self.action_server_scan.start()

        action_name = 'grasp_action'
        self.action_server_grasp = actionlib.SimpleActionServer(action_name, ScanSceneAction, execute_cb=self.grasp_cb, auto_start=False)
        self.action_server_grasp.start()

    def scan_cb(self, goal):
        rospy.loginfo("Scanning action was triggered")
        result = ScanSceneResult()

        grasp_pose = PoseStamped()

        rospy.loginfo("Succeed")
        result.selected_grasp_pose = grasp_pose
        self.action_server_scan.set_succeeded(result)

    def grasp_cb(self, goal):
        rospy.loginfo("Grasping action was triggered")
        result = ScanSceneResult()

        grasp_pose = PoseStamped()

        rospy.loginfo("Succeed")
        result.selected_grasp_pose = grasp_pose
        self.action_server_grasp.set_succeeded(result)

def main():
    rospy.init_node('dummy_actions_node')

    das = DummyActionServers()

    rospy.spin()


if __name__ == '__main__':
    main()
