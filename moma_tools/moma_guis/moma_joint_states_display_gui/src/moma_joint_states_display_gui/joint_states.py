...
class JointStates(Plugin):
    ...
    def __init__(self, context):
        ...
        self.controller_name = rospy.get_param(
            "/moma_joint_states_display_gui/controller_name"
        )
        self.controller_manager_ns = rospy.get_param(
            "/moma_joint_states_display_gui/controller_namespace", ""
        )
        ...
        self.joints = dict(
            zip(self.joint_names, zip(self.lower_limits, self.upper_limits))
        )

        self._state_widgets = dict()
        states_table = self._widget.states_table
        states_table.setHorizontalHeaderLabels(
            ["Joint", "Lower Limit", "Position", "Upper Limit"]
        )
    ...
    def _on_joint_state(self, msg):
        for name, position in zip(msg.name, msg.position):
            if name in self._state_widgets:
                ...
                margin = min(
                    position - self.joints[name][0], self.joints[name][1] - position
                )
                # Fading to yellow if between safety_margin and twice safety_margin
                color_brightness_blue = max(
                    min((margin - self.safety_margin) / self.safety_margin, 1) * 255, 0
                )
                ...
                self._state_widgets[name].setBackground(
                    QColor(255, color_brightness_green, color_brightness_blue)
                )
